...
public class TransactedExample {
    ...
    public static String supplierOrderTopicName = null;
    ...
    public static void run_threads(int quantity) {
        ...
        Vendor v = new Vendor();
        GenericSupplier ms =
            new GenericSupplier("Monitor", supplierOrderTopicName);
        GenericSupplier ss =
            new GenericSupplier("Hard Drive", supplierOrderTopicName);
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 1) {
            ...
            System.err.println("Program assumes three " +
                "queues named jms/AQueue, jms/BQueue, and jms/CQueue " +
                "and one topic named jms/OTopic");
            System.exit(1);
        }

        te.vendorOrderQueueName = new String("jms/AQueue");
        te.retailerConfirmQueueName = new String("jms/BQueue");
        te.supplierOrderTopicName = new String("jms/OTopic");
        te.vendorConfirmQueueName = new String("jms/CQueue");
    ...
    public static class Retailer extends Thread {
        ...
        public void run() {
            ...
            javax.jms.Queue vendorOrderQueue = null;
            javax.jms.Queue retailerConfirmQueue = null;
            ...
            try {
                ...
                vendorOrderQueue = SampleUtilities.getQueue(vendorOrderQueueName,
                        session);
                retailerConfirmQueue = SampleUtilities.getQueue(retailerConfirmQueueName,
                        session);
            } catch (Exception e) {
    ...
    public static class Vendor extends Thread {
        ...
        public void run() {
            ...
            javax.jms.Queue vendorOrderQueue = null;
            Topic supplierOrderTopic = null;
            javax.jms.Queue vendorConfirmQueue = null;
            ...
            try {
                ...
                vendorOrderQueue = SampleUtilities.getQueue(vendorOrderQueueName,
                        session);
                supplierOrderTopic = SampleUtilities.getTopic(supplierOrderTopicName,
                        session);
                vendorConfirmQueue = SampleUtilities.getQueue(vendorConfirmQueueName,
                        session);
            } catch (Exception e) {
                System.err.println("Connection problem: " + e.toString());
                System.err.println("Program assumes three " +
            ...
            try {
                 ...
                 */
                vendorConfirmReceiver = asyncSession.createConsumer(vendorConfirmQueue);
    ...
    public static class GenericSupplier extends Thread {
         ...
         * @param inTopic     the topic from which the order is
         *                    obtained
         */
        public GenericSupplier(String itemName, String inTopic) {
            PRODUCT_NAME = itemName;
            IN_ORDER_TOPIC = inTopic;
        }
        ...
        public void run() {
            ...
            try {
                ...
                connection = connectionFactory.createConnection();
                session = connection.createSession(true, 0);
                orderTopic = SampleUtilities.getTopic(IN_ORDER_TOPIC, session);
            } catch (Exception e) {
                System.err.println("Connection problem: " + e.toString());
                System.err.println("Program assumes three " +
                    "queues named jms/AQueue, jms/BQueue, and jms/CQueue " +
            ...
            while (true) {
                try {
                    ...
                    if (inMessage instanceof MapMessage) {
                         ...
                         */
                        MessageProducer producer =
                            session.createProducer(inMessage.getJMSReplyTo());
                        producer.send(session.createMessage());
                        session.commit();
                     ...
                     */
                    MessageProducer producer =
                        session.createProducer((javax.jms.Queue) orderMessage.getJMSReplyTo());
                    outMessage = session.createMapMessage();
